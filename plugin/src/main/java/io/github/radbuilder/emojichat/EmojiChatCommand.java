...
class EmojiChatCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
		if (args.length < 1) {
			sender.sendMessage(ChatColor.AQUA + "EmojiChat v2, a fork by Vap0r1ze");
		...
		switch (args[0].toLowerCase()) {
			...
			case "resourcepack":
				...
				if (sender instanceof Player) {
					((Player) sender).setResourcePack(plugin.getEmojiHandler().getPackVariant().getUrl(plugin.getConfig().getString("pack-quality"), plugin.getDescription().getVersion()));
